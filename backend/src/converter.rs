...
use crate::types::{ConversionOptions, ConversionResult, Lab, ProgressEvent};
use crate::voxelizer;
...
pub async fn convert(
    ...
    send(
        0.50,
        &format!("Voxelized: {n_voxels} surface voxels ({sx}×{sy}×{sz})"),
    );
    ...
    let (block_names, unique): (Vec<&str>, usize) = if options.use_color_sampling {
        ...
        let lab_queries: Vec<Lab> = dithered
            .par_iter()
        ...
        send(
            0.65,
            &format!("Block matching complete: {unique} unique blocks"),
        );
        (names, unique)
    } else {
...
pub async fn preview_litematic(
    input_path: &str,
    output_path: &str,
    options: &ConversionOptions,
    palette: &Palette,
) -> Result<PreviewSchematic, String> {
    let vox = voxelizer::voxelize(
        input_path,
        options.max_size,
        options.voxel_size,
    ...
    let colors = if options.use_dithering {
        dithering::apply_dithering(&coords, &colors, 28.0)
    } else {
        colors
    };
    let names: Vec<&str> = if options.use_color_sampling {
        let queries: Vec<Lab> = colors
            .par_iter()
            .map(|c| crate::palette::rgb_to_lab_vec(*c))
            .collect();
        palette
            .match_indices_batch(&queries, 7)
            .iter()
            .map(|&i| palette.name_of(i))
            .collect()
    } else {
        vec![options.default_block_name.as_str(); coords.len()]
    };
    crate::litematic::write_litematic(
        output_path,
        &coords,
