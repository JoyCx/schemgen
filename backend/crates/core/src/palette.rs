...
pub fn rgb_to_lab_vec(rgb: [f32; 3]) -> Lab {
...
pub fn lab_to_rgb(lab: &Lab) -> Rgb {
    ...
    let gl = x * -0.969266 + y * 1.8760108 + z * 0.0415560;
...
pub fn ciede2000_pre(l1: &LabTerms, l2: &LabTerms) -> f32 {
    ...
    let dh = if c1p * c2p == 0. {
        ...
        2. * (c1p * c2p).sqrt() * (dh2.to_radians() * 0.5).sin()
    };
    let lp = (ll1 + ll2) * 0.5;
    let cp = (c1p + c2p) * 0.5;
    let hp = if c1p * c2p == 0. {
    ...
    let t = 1. - 0.17 * ((hp - 30.).to_radians()).cos()
        + 0.24 * ((2. * hp).to_radians()).cos()
        + 0.32 * ((3. * hp + 6.).to_radians()).cos()
        - 0.20 * ((4. * hp - 63.).to_radians()).cos();
...
impl Palette {
    ...
    pub fn from_table(table: &ColorTable) -> Result<Self> {
        ...
        let terms = pts
            .iter()
            .map(|&[l, a, b]| LabTerms::from_parts(l, a, b))
            .collect();
        let mut by_l: Vec<u32> = (0..pts.len() as u32).collect();
        by_l.sort_by(|&x, &y| {
            pts[x as usize][0]
                .total_cmp(&pts[y as usize][0])
                .then(x.cmp(&y))
        });
    ...
    fn match_bright(&self, query: &Lab, q_terms: &LabTerms, minimum_l: f32) -> Option<usize> {
        ...
        loop {
            ...
            if best_i != usize::MAX && dl * dl > L_BOUND * best_d as f64 {
                break;
            }
...
pub(crate) mod tests {
    ...
    fn test_pal() {
        ...
        assert_eq!(p.len(), 2);
        let best = p.match_block_index(&rgb_to_lab(128., 128., 128.), 5);
        assert_eq!(p.name_of(best as u32), "minecraft:stone");
    }
    ...
    fn test_bright_pruning_matches_linear_scan() {
        ...
        assert!(
            checked > 10_000,
            "expected a broad sweep, checked {checked}"
        );
    }
    ...
    fn test_real_palette_matches_reference() {
        ...
        assert!(
            bright > 1000 && dark > 1000,
            "both paths must be exercised: {bright}/{dark}"
        );
    }
